...
def load_single_instance_from_line(istest: bool , line: str):
    """
    ...
    if istest:
        ...
        label = split_line[1]
        warrant0 = split_line[2]
        warrant1 = split_line[3]
        reason = split_line[4]
        claim = split_line[5]
        debate_title = split_line[6]
        debate_info = split_line[7]
        ...
        return instance_id, warrant0, warrant1, label, reason, claim, debate_meta_data
    else:
        # "#id warrant0 warrant1 correctLabelW0orW1 reason claim debateTitle debateInfo
        assert len(split_line) == 8

        instance_id = split_line[0]
        warrant0 = split_line[1]
        warrant1 = split_line[2]
        correct_label_w0_or_w1 = split_line[3]
        reason = split_line[4]
        claim = split_line[5]
        debate_title = split_line[6]
        debate_info = split_line[7]
...
def preprocess_dataset_swag(path, is_test_set=False):
    start_time = time.time()
    writer = open(path  + '-processed_12_13.json', 'w', encoding='utf-8') 
    n = 0
    ex_cnt = 0
    birth_data = []
    with open(path,'r', encoding='utf-8') as csvfile:
        ...
        for row in csv_reader:  # 将csv 文件中的数据保存到birth_data中
            ...
            #print(row[3],row[7],row[8],row[9],row[10])
            birth_data.append(row)
    #print(birth_data)   
    d_id=0 
    for row in birth_data:
        label0 = '1'
        label4=label1=label2=label3='0'
        #print(row)
        d_id += 1
        startphrasee,ending0,ending1,ending2,ending3,ending4 = row[2],row[3],row[4],row[5],row[6],row[7]
        #print(startphrasee,ending0,ending1,ending2,ending3,label)
        d_dict = tokenize(startphrasee)
        q_dict = tokenize('swagdata')
        c_dict0 = tokenize(ending0)
        c_dict1 = tokenize(ending1)
        c_dict2 = tokenize(ending2)
        c_dict3 = tokenize(ending3)
        c_dict4 = tokenize(ending4)
        '''
        if(label=='0'):
            label0=1
        if(label=='1'):
        ...
        if(label=='3'):
            label3=1
        '''
        example = get_example(str(d_id) + '_'+str(0), d_dict, q_dict, c_dict0,label0)
        ...
        ex_cnt += 1
        example = get_example(str(d_id) + '_' + str(1), d_dict, q_dict, c_dict1,label1)
        ...
        if ex_cnt % 3000 ==0:
            print("processing %d ..."%(ex_cnt))
        example = get_example(str(d_id) + '_' + str(2), d_dict, q_dict, c_dict2,label2)
        ...
        if ex_cnt % 3000 ==0:
            print("processing %d ..."%(ex_cnt))
        example = get_example(str(d_id) + '_'  + str(3), d_dict, q_dict, c_dict3,label3)
        ...
        if ex_cnt % 3000 ==0:
            print("processing %d ..."%(ex_cnt))
        example = get_example(str(d_id) + '_'+str(4), d_dict, q_dict, c_dict4,label4)
        example.update(compute_features(d_dict, q_dict, c_dict4))
        writer.write(json.dumps(example))
        writer.write('\n')
        ex_cnt += 1
        if ex_cnt % 3000 ==0:
...
if __name__ == '__main__':
    ...
    preprocess_dataset_swag('/home/hd_1T/haiou/newdata/data/swag/train_full.csv')
    #preprocess_dataset_swag('/home/hd_1T/haiou/newdata/data/swag/val_full.csv')
    #preprocess_dataset_swag('/home/hd_1T/haiou/newdata/data/swag/')
    #preprocess_dataset_snli('/home/hd_1T/haiou/newdata/data/snli_1.0/test.txt')
    #preprocess_dataset_argument('./data/argument/SemEval2018--argument_reasoning_comprehension/model_code/data/test_answer.txt',True)
    #preprocess_dataset_argument('./data/argument/SemEval2018--argument_reasoning_comprehension/model_code/data/train/train-full.txt')
    #---high
