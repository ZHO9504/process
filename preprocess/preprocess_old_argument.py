...
def preprocess_dataset_argument(path, is_test_set=False):
    start_time = time.time()
    writer = open(path  + '-processed_nodebate.json', 'w', encoding='utf-8') 
    n = 0
    ex_cnt = 0
    for line in open(path, 'r', encoding='utf-8'):
        if n == 0:
            ...
            continue
        label0 = label1 = '0'
        #错误 debate reason claim 都是什么关系？？？
        instance_id, warrant0, warrant1, label, reason, claim, debate_meta_data = \
            load_single_instance_from_line(is_test_set, line)
        if label == '-1':
            label = anss[instance_id]
        if label == '0':
            label0 = '1'
        if label == '1':
            label1 = '1'
        ...
        d_dict = tokenize(reason)
        q_dict = tokenize(claim)
        c_dict0 = tokenize(warrant0)
        c_dict1 = tokenize(warrant1)
        example = get_example(instance_id + '_0', d_dict, q_dict, c_dict0,label0)
        example.update(compute_features(d_dict, q_dict, c_dict0))
        writer.write(json.dumps(example))
        writer.write('\n')
        example = get_example(instance_id + '_1', d_dict, q_dict, c_dict1,label1)
...
if __name__ == '__main__':
    ...
    preprocess_dataset_argument('./data/argument/SemEval2018--argument_reasoning_comprehension/model_code/data/dev/dev-full.txt')
