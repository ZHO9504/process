...
def preprocess_dataset_qald(path, is_test_set=False):
    start_time = time.time()
    writer_multi = open(path  + '-processed.json', 'w', encoding='utf-8')
    ex_cnt = 0
    num = 0
    import re
    with open(path, 'r') as json_file:
        ...
        for qq in qusss:
            ...
            for qqs in qs:
                if qqs['language'] == 'en':
                    q = qqs['string']
                    if len(re.findall( r'\w\W',  q)) > 0: 
                        qbaby = re.sub('\W','',q) 
                    qkeys = qqs['keywords'].split(', ')
                    qwords = q.split()
                    label = ''
                    keys = {}
                    ...
                    for i, word in enumerate(qwords):
                        for key in keys:
                            if word == key or word[:len(key)-1] == key:
                                label = label + '1 '
                                print (word,word[:len(key)-1],key)
                            else:
                                label = label + '0 '
        if not len(label.split()) == len(q.split()):
            print(q,label)
        q_dict = tokenize(q)
        d_dict = tokenize('qald')
        c_dict = tokenize('qald')
...
if __name__ == '__main__':
    ...
    preprocess_dataset_qald('./data/qald/qald-9-train-multilingual.json')
