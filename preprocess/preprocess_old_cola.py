...
def preprocess_dataset_cola(path, is_test_set=False):
    start_time = time.time()
    writer = open(path  + '-processed_12_04.json', 'w', encoding='utf-8') 
    n = 0
    ex_cnt = 0
    for line in open(path, 'r', encoding='utf-8'):
        if n == 0:
            n += 1 
            continue
        id1, label, warrant0 = line.strip().split('\t')
        if label != '1' and label != '0':
            continue
        #print(reason)
        print(warrant0, label)
        d_dict = tokenize(warrant0)
        q_dict = tokenize('coladata')
        c_dict = tokenize('coladata')
        example = get_example(str(ex_cnt) + '_'+str(ex_cnt), d_dict, q_dict, c_dict,label)
...
if __name__ == '__main__':
    ...
    preprocess_dataset_cola('/home/hd_1T/haiou/newdata/data/cola/in_domain_dev.tsv')
    preprocess_dataset_cola('/home/hd_1T/haiou/newdata/data/cola/in_domain_train.tsv')
    preprocess_dataset_cola('/home/hd_1T/haiou/newdata/data/cola/out_of_domain_dev.tsv')
